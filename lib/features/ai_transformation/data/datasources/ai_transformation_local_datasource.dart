...
class AITransformationLocalDataSource {
  ...
  Future<void> saveTransformation(
    TransformationResultModel transformation,
  ) async {
    try {
      AppLogger.info(
        'Saving transformation to local database: ${transformation.id}',
      );
  ...
  Future<List<TransformationResultModel>> getTransformationHistory() async {
    try {
      ...
      AppLogger.info(
        'Found ${transformations.length} transformations in history',
      );
      return transformations;
    } catch (e) {
      AppLogger.error('Failed to fetch transformation history: $e');
      throw CacheException(
        'Failed to fetch transformation history: ${e.toString()}',
      );
    }
  }
  ...
  Future<String> downloadTransformedImage(
    ...
    try {
      ...
      AppLogger.info(
        'Image downloaded successfully: $fileName ($fileSize bytes)',
      );
      ...
      throw NetworkException(
        'Failed to download image: ${e.message ?? 'Unknown error'}',
      );
    } catch (e) {
  ...
  Future<void> deleteTransformation(String transformationId) async {
    try {
      ...
      if (transformations.isNotEmpty) {
        final transformation = TransformationResultModel.fromJson(
