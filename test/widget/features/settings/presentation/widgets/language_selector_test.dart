...
void main() {
  group('LanguageSelector Widget Tests', () {
    group('with real providers (TDD test for SharedPreferences issue)', () {
      setUp(() async {
      ...
      testWidgets(
        'should handle language selection without SharedPreferences error',
        (tester) async {
          // RED: This test should initially fail with SharedPreferences error
          // Arrange
          await tester.pumpWidget(
            const TestAppWrapper(child: Scaffold(body: LanguageSelector())),
          );
          await tester.pumpAndSettle();

          // Act - Find and tap the English language option
          final englishOption = find
              .byType(InkWell)
              .at(1); // Second option is English
          expect(englishOption, findsOneWidget);

          // This should not throw a SharedPreferences error
          await tester.tap(englishOption);
          await tester.pumpAndSettle();

          // Assert - Verify no error occurred and snackbar appeared
